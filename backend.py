...
Automatically uses PostgreSQL on Render and SQLite on local laptop — no manual switching needed.
"""
...
# ── DATABASE SETUP ──
# If DATABASE_URL exists (on Render) use PostgreSQL
# If not (on laptop) use SQLite automatically
...
# ── DB CONNECTION ──
# Returns PostgreSQL connection on Render, SQLite on laptop
def db():
    if USE_PG:
...
# ── QUERY HELPER ──
# SQLite uses ? for parameters, PostgreSQL uses %s — this fixes it automatically
def ph(sql):
...
def db_fetch(sql,params=()):
    ...
    try:
        if USE_PG:
            cur=conn.cursor();cur.execute(ph(sql),params);rows=cur.fetchall();cur.close()
        else:
            rows=conn.execute(ph(sql),params).fetchall()
        return rows
    finally:
        conn.close()
...
def db_fetchone(sql,params=()):
    ...
    try:
        if USE_PG:
            cur=conn.cursor();cur.execute(ph(sql),params);row=cur.fetchone();cur.close()
        else:
            row=conn.execute(ph(sql),params).fetchone()
        return row
    finally:
        conn.close()
...
def db_execute(sql,params=()):
    ...
    try:
        if USE_PG:
            cur=conn.cursor();cur.execute(ph(sql),params);conn.commit();cur.close()
        else:
            conn.execute(ph(sql),params);conn.commit()
    finally:
        conn.close()

# ── INIT DATABASE ──
# Creates tables if they don't exist — works for both PostgreSQL and SQLite
def init_db():
    conn=db()
    try:
        if USE_PG:
            cur=conn.cursor()
            cur.execute('''CREATE TABLE IF NOT EXISTS users(id TEXT PRIMARY KEY,username TEXT UNIQUE NOT NULL,password_hash TEXT NOT NULL,created_at TEXT DEFAULT CURRENT_TIMESTAMP,last_login TEXT)''')
            cur.execute('''CREATE TABLE IF NOT EXISTS runs(id TEXT PRIMARY KEY,user_id TEXT NOT NULL,unit_w INTEGER,unit_d INTEGER,unit_h INTEGER,algorithm TEXT,boxes_json TEXT,results_json TEXT,efficiency REAL,units_used INTEGER,boxes_placed INTEGER,runtime_ms REAL,created_at TEXT DEFAULT CURRENT_TIMESTAMP)''')
            cur.execute('''CREATE TABLE IF NOT EXISTS datasets(id TEXT PRIMARY KEY,user_id TEXT NOT NULL,name TEXT,boxes_json TEXT,unit_config TEXT,created_at TEXT DEFAULT CURRENT_TIMESTAMP)''')
...
# Migrate old SQLite schema if needed — only runs on laptop
def migrate_db():
    if USE_PG:return
    try:
        import sqlite3 as s3
        conn=s3.connect('cryopack.db')
