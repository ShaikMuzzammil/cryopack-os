...
Flask + SQLite | Auth | 3D Bin Packing | Dataset Persistence
"""
...
app.config.update(SECRET_KEY=os.environ.get('SECRET_KEY','cryopack-os-secret-2024-vaccine-logistics'),SESSION_COOKIE_HTTPONLY=True,SESSION_COOKIE_SECURE=False,SESSION_COOKIE_SAMESITE='Lax',SESSION_COOKIE_NAME='cryopack_session',PERMANENT_SESSION_LIFETIME=timedelta(days=30))
CORS_HEADERS={'Access-Control-Allow-Credentials':'true','Access-Control-Allow-Headers':'Content-Type, Authorization','Access-Control-Allow-Methods':'GET, POST, DELETE, OPTIONS'}
@app.after_request
def add_cors(r):
    origin=request.headers.get('Origin','');r.headers['Access-Control-Allow-Origin']=origin or '*'
...
def init_db():
    with db() as c:
        c.executescript('''
            ...
            CREATE TABLE IF NOT EXISTS runs(id TEXT PRIMARY KEY,user_id TEXT NOT NULL,unit_w INTEGER,unit_d INTEGER,unit_h INTEGER,algorithm TEXT,boxes_json TEXT,results_json TEXT,efficiency REAL,units_used INTEGER,boxes_placed INTEGER,runtime_ms REAL,created_at TEXT DEFAULT CURRENT_TIMESTAMP,FOREIGN KEY(user_id) REFERENCES users(id));
            CREATE TABLE IF NOT EXISTS datasets(id TEXT PRIMARY KEY,user_id TEXT NOT NULL,name TEXT,boxes_json TEXT,unit_config TEXT,created_at TEXT DEFAULT CURRENT_TIMESTAMP,FOREIGN KEY(user_id) REFERENCES users(id));
        ''')
init_db()
def migrate_db():
    try:
        with db() as c:
            ...
            if 'unit_b' in cols and 'unit_d' not in cols:
                c.executescript('''
                    CREATE TABLE runs_new(id TEXT PRIMARY KEY,user_id TEXT NOT NULL,unit_w INTEGER,unit_d INTEGER,unit_h INTEGER,algorithm TEXT,boxes_json TEXT,results_json TEXT,efficiency REAL,units_used INTEGER,boxes_placed INTEGER,runtime_ms REAL,created_at TEXT DEFAULT CURRENT_TIMESTAMP,FOREIGN KEY(user_id) REFERENCES users(id));
...
def serve_static(filename):return send_from_directory('.',filename) if os.path.exists(filename) else({'error':'Not found'},404)
# Auth decorator — returns 401 if not logged in
def require_login(f):
    @wraps(f)
    def dec(*a,**kw):
...
def register():
    ...
    try:
        ...
        with db() as c:c.execute('INSERT INTO users(id,username,password_hash) VALUES(?,?,?)',(user_id,u,generate_password_hash(p)))
        session.update({'user_id':user_id,'username':u});session.permanent=True
...
def login():
    ...
    with db() as c:
        ...
        if row and check_password_hash(row[1],p):c.execute('UPDATE users SET last_login=? WHERE id=?',(datetime.now().isoformat(),row[0]))
    if row and check_password_hash(row[1],p):
        session.update({'user_id':row[0],'username':u});session.permanent=True
        return jsonify({'success':True,'user_id':row[0],'username':u}),200
    return jsonify({'error':'Invalid credentials'}),401
@app.route('/api/auth/logout',methods=['POST'])
def logout():session.clear();return jsonify({'success':True}),200
@app.route('/api/auth/status')
def auth_status():
    if 'user_id' in session:return jsonify({'authenticated':True,'username':session.get('username'),'user_id':session.get('user_id')}),200
    return jsonify({'authenticated':False}),200
# ══════════════════════════════════════════════════
# 3D BIN PACKING ALGORITHMS
# ══════════════════════════════════════════════════
def can_place(pls,bw,bd,bh,px,py,pz,uw,ud,uh):
    if px+bw>uw or py+bd>ud or pz+bh>uh:return False
...
def get_rotations(bw,bd,bh):
    ...
    return out
# Generate candidate placement positions (corner-points)
def get_candidates(pls,uw,ud,uh):
    c={(0,0,0)}
    for p in pls:c.update([(p['x']+p['w'],p['y'],p['z']),(p['x'],p['y']+p['d'],p['z']),(p['x'],p['y'],p['z']+p['h'])])
    return list(c)
...
def try_place_default(pls,bw,bd,bh,uw,ud,uh):
    ...
    for(x,y,z) in cands:
        if can_place(pls,bw,bd,bh,x,y,z,uw,ud,uh):return(x,y,z)
    return None
# BFD/HYBRID: gravity sort — lowest z first
def try_place_gravity(pls,bw,bd,bh,uw,ud,uh):
    cands=sorted(get_candidates(pls,uw,ud,uh),key=lambda c:(c[2],-(c[0]+c[1])))
    for(x,y,z) in cands:
        if can_place(pls,bw,bd,bh,x,y,z,uw,ud,uh):return(x,y,z)
    return None
# FF: spread sort — minimize stacking height
...
# Core packing loop — runs selected algorithm on box list
def run_algorithm(algo,uw,ud,uh,boxes):
    t0=time.time();units=[];all_pl=[]
    if {'HYBRID':True,'BFD':True,'FFD':True,'FF':False}.get(algo,False):boxes=sorted(boxes,key=lambda b:b['w']*b['d']*b['h'],reverse=True)
    for idx,box in enumerate(boxes):
        bw,bd,bh=box['w'],box['d'],box['h'];rots=get_rotations(bw,bd,bh);best=None
        if algo=='HYBRID':
            bvol=float('inf')
            for ui,up in enumerate(units):
                used=sum(p['w']*p['d']*p['h'] for p in up);rem=uw*ud*uh-used
                if rem<bw*bd*bh:continue
                for(rw,rd,rh) in rots:
                    pos=try_place_gravity(up,rw,rd,rh,uw,ud,uh)
                    if pos and rem<bvol:bvol,best=rem,(ui,pos,rw,rd,rh)
        elif algo=='BFD':
            uvol=uw*ud*uh;br=float('inf')
            for ui,up in enumerate(units):
                used=sum(p['w']*p['d']*p['h'] for p in up);rem=uvol-used
                if rem<bw*bd*bh:continue
                for(rw,rd,rh) in rots:
                    pos=try_place_gravity(up,rw,rd,rh,uw,ud,uh)
                    if pos and rem<br:br,best=rem,(ui,pos,rw,rd,rh)
        elif algo=='FFD':
            for ui,up in enumerate(units):
                pos=try_place_default(up,bw,bd,bh,uw,ud,uh)
                if pos:up.append({'x':pos[0],'y':pos[1],'z':pos[2],'w':bw,'d':bd,'h':bh,'box_idx':idx,'unit_id':ui});all_pl.append({'box':box,'x':pos[0],'y':pos[1],'z':pos[2],'unit_id':ui,'box_idx':idx,'rotated':False});best=True;break
        else:  # FF
            for ui,up in enumerate(units):
                pos=try_place_spread(up,bw,bd,bh,uw,ud,uh)
                if pos:up.append({'x':pos[0],'y':pos[1],'z':pos[2],'w':bw,'d':bd,'h':bh,'box_idx':idx,'unit_id':ui});all_pl.append({'box':box,'x':pos[0],'y':pos[1],'z':pos[2],'unit_id':ui,'box_idx':idx,'rotated':False});best=True;break
        if best and algo in('HYBRID','BFD') and isinstance(best,tuple):
            ui,pos,rw,rd,rh=best;pb={'w':rw,'d':rd,'h':rh}
            ...
            placer=PLACERS[algo];bp,br2=None,(bw,bd,bh)
            for(rw,rd,rh) in rots:
                pos=placer([],rw,rd,rh,uw,ud,uh)
                if pos:bp,br2=pos,(rw,rd,rh);break
            bp=bp or(0,0,0);rw,rd,rh=br2;nuid=len(units)
...
def prep_boxes(boxes_data,uw,ud,uh):
    ...
    for b in boxes_data:
        for _ in range(int(b.get('qty',1))):
            bw,bd,bh=int(b['w']),int(b.get('d',b.get('b',1))),int(b['h']);seen,uniq=set(),[]
            for d in[(bw,bd,bh),(bw,bh,bd),(bd,bw,bh),(bd,bh,bw),(bh,bw,bd),(bh,bd,bw)]:
    ...
    return out
# Calculate volumetric efficiency as percentage
def calc_eff(pls,n_units,uw,ud,uh):
    if not n_units:return 0
    return round(sum(p['box']['w']*p['box']['d']*p['box']['h'] for p in pls)/(n_units*uw*ud*uh)*100,2)
# ── Pack ──
@app.route('/api/pack',methods=['POST'])
@require_login
def pack():
    d=request.json;uw,ud,uh=int(d.get('unit_w',10)),int(d.get('unit_d',10)),int(d.get('unit_h',10))
    algo=d.get('algorithm','HYBRID');boxes_flat=prep_boxes(d.get('boxes',[]),uw,ud,uh)
    pls,n_units,rt=run_algorithm(algo,uw,ud,uh,boxes_flat);eff=calc_eff(pls,n_units,uw,ud,uh)
    pl_data=[{'box':p['box'],'x':p['x'],'y':p['y'],'z':p['z'],'unit_id':p['unit_id'],'box_idx':p['box_idx'],'rotated':p.get('rotated',False)} for p in pls]
    run_id=str(uuid.uuid4())
    with db() as c:c.execute('INSERT INTO runs(id,user_id,unit_w,unit_d,unit_h,algorithm,boxes_json,results_json,efficiency,units_used,boxes_placed,runtime_ms) VALUES(?,?,?,?,?,?,?,?,?,?,?,?)',(run_id,uid(),uw,ud,uh,algo,json.dumps(d.get('boxes',[])),json.dumps(pl_data),eff,n_units,len(pls),rt))
...
@require_login
def get_runs():
    with db() as c:rows=c.execute('SELECT id,algorithm,efficiency,units_used,boxes_placed,runtime_ms,created_at,unit_w,unit_d,unit_h FROM runs WHERE user_id=? ORDER BY created_at DESC LIMIT 50',(uid(),)).fetchall()
...
@require_login
def get_run(run_id):
    with db() as c:r=c.execute('SELECT id,algorithm,efficiency,units_used,boxes_placed,runtime_ms,created_at,unit_w,unit_d,unit_h,boxes_json,results_json FROM runs WHERE id=? AND user_id=?',(run_id,uid())).fetchone()
...
def delete_run(run_id):
    with db() as c:c.execute('DELETE FROM runs WHERE id=? AND user_id=?',(run_id,uid()))
    return jsonify({'success':True}),200
@app.route('/api/runs/all',methods=['DELETE'])
@require_login
def delete_all_runs():
    with db() as c:c.execute('DELETE FROM runs WHERE user_id=?',(uid(),))
    return jsonify({'success':True}),200
# ── Datasets ──
@app.route('/api/datasets',methods=['POST'])
@require_login
def save_dataset():
    d=request.json;did=str(uuid.uuid4())
    with db() as c:c.execute('INSERT INTO datasets(id,user_id,name,boxes_json,unit_config) VALUES(?,?,?,?,?)',(did,uid(),d.get('name','Unnamed'),json.dumps(d.get('boxes',[])),json.dumps(d.get('unit_config',{}))))
    return jsonify({'id':did,'success':True}),201
@app.route('/api/datasets',methods=['GET'])
@require_login
def get_datasets():
    with db() as c:rows=c.execute('SELECT id,name,boxes_json,unit_config,created_at FROM datasets WHERE user_id=? ORDER BY created_at DESC',(uid(),)).fetchall()
...
@require_login
def get_dataset(did):
    with db() as c:r=c.execute('SELECT id,name,boxes_json,unit_config FROM datasets WHERE id=? AND user_id=?',(did,uid())).fetchone()
    if not r:return jsonify({'error':'Not found'}),404
...
def delete_dataset(did):
    with db() as c:c.execute('DELETE FROM datasets WHERE id=? AND user_id=?',(did,uid()))
    return jsonify({'success':True}),200
# ── Analysis ──
