...
[![Live Demo](https://img.shields.io/badge/🌐_Live_Demo-cryopack--os.onrender.com-00b4d8?style=for-the-badge)](https://cryopack-os.onrender.com)
...
[![License](https://img.shields.io/badge/License-Private-red?style=for-the-badge)](https://github.com/ShaikMuzzammil/cryopack-os)
...
| 6 | **Dataset Persistence** | Save box configurations and reload them anytime |
| 7 | **Run History** | All packing results saved with metrics |
| 8 | **Cross‑Device Sync** | Data available on all your devices via cloud database |
...
| **Current queue** (boxes waiting to be packed) | ⚠️ **Not auto‑synced** | Save the queue as a dataset (💾 button), then load it on another device |
...
1. **You log in** — your browser gets a secure session cookie (fixed for mobile with `Secure=True` and `SameSite=None`)
...
- 🖥️ **On desktop** — design a box configuration for a new vaccine shipment, test with 10 algorithms, save the best dataset
...
| Session cookies | `HttpOnly`, `Secure`, `SameSite=Lax/None` — protected against XSS and CSRF |
...
> **This works because we fixed the mobile cookie issue** — changed `SESSION_COOKIE_SECURE=True` and `SESSION_COOKIE_SAMESITE='None'` in the backend. Without these, mobile browsers reject the session cookie.
...
| Panel | Contents |
|:-----:|---------|
| **Left** | Unit config, box input, queue, run controls |
| **Center** | Dual 3D canvas (storage top, conveyor belt bottom) with draggable divider |
| **Right** | Live metrics, unit breakdown, placement log |
...
[![GitHub](https://img.shields.io/badge/GitHub-ShaikMuzzammil-181717?style=for-the-badge&logo=github)](https://github.com/ShaikMuzzammil)
[![Live Site](https://img.shields.io/badge/Live_Site-cryopack--os.onrender.com-00b4d8?style=for-the-badge&logo=render)](https://cryopack-os.onrender.com)
[![Repository](https://img.shields.io/badge/Repository-cryopack--os-2ea44f?style=for-the-badge&logo=github)](https://github.com/ShaikMuzzammil/cryopack-os)
